...
Function DecodeCHeader(path:String)
	...
	header=LoadString(path,true)
			
	For line=Eachin header.Split("~n")
		'output+="~r~n"+ "------------------------------------"
		'output+="~r~n"+ ">>"+line
			...
			If inclass=2 And line.Length
				...
				If b0>0
					If s>0	
						...
						Else
							output+="~r~n"+ "~t'Method "+f+"<T>"+t+"("+args.Join(",")+")"+"' This method Has generics !!!!! wrappit?"
							If isvirtual output+=" virtual"
							If isoverride output+=" override"
						end
				...
				Else
					...
					If b0>0
						If s>0				
							...
							Else
								'output+="~r~n"+ "'Function "+f+" is with generics"
								output+="~r~n"+ "'Function "+f+"<T>"+t+"("+args.Join(",")+")"
								Return true 
							end
		...
		If line.StartsWith("struct ")
			...
			For Local n:=Eachin extendedstructsindex
				If n.Key=templine 'on est a la ligne de déclaration de la struct étendue
					...
					For Local linebis:=Eachin header.Split("~n")
...
Function Main()
	...
	ChangeDir("C:\Users\W7Son\Desktop\b2dJson-master\c++")
	...
	output+="~r~n"
	Local enums:=LoadString("box2dEnums.monkey2",true)
	'Print enums
	output+=enums
	output+="~r~n"
	output+="~r~n"
